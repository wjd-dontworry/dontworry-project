...
export const fetchBoards = () => {
  return async (dispatch: Dispatch<BoardActions>, getState: () => RootState) => {
    try {
      ...
      const { data, error } = await supabase.from("board").select("*, user(*)").returns<BoardWithLike[]>()
      if (error) {
...
export const createBoard = (title: string, content: string, user_id: string) => {
  return async (dispatch: Dispatch<BoardActions>) => {
    try {
      const { data, error } = await supabase.from("board").insert({ title: title, content: content, user_id: user_id }).select()
      if (error) {
...
export const updateBoard = (boardId: number, title: string, content: string) => {
  return async (dispatch: Dispatch<BoardActions>) => {
    try {
      const { data, error } = await supabase.from("board").update({ title: title, content: content }).eq("board_id", boardId).select()
      if (error) {
        console.log(error)
