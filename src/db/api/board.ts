...
export const fetchBoard = async (orderBy: string, ascending: boolean) => {
  try {
    const boardWithUserQuery = supabase.from("board").select("*, user(*), board_like(*)").order(orderBy, { ascending })
...
export const getBoardLikeCount = async (boardId: number) => {
  try {
    const { error, count } = await supabase.from("board_like").select("*", { count: "exact", head: true }).eq("board_id", boardId)
...
export const existBoardLikeId = async (boardId: number, userId: string) => {
  try {
    const { data, error } = await supabase.from("board_like").select("like_id").eq("board_id", boardId).eq("user_id", userId)
