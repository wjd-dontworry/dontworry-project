...
export default function ChallengeCreateScreen() {
  ...
  const createParticipation = async () => {
    try {
      const { data, error } = await supabase.from("challenge_participation").insert([{ start_date: "2024-05-01", end_date: "2024-05-31", user_id: user?.id, challenge_id: route.params!.challengeId }])
