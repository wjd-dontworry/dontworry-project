...
export default function ChallengeCreateScreen() {
  ...
  const [challengeList, setChallengeList] = useState([{ id: 0, time: "", taskname: "" }])

  const onChangeItemInput = (id: number, name: string, value: string) => {
    setChallengeList(challengeList.map(item => (item.id === id ? { ...item, [name]: value } : item)))
  }
  ...
  const createTasks = async (challengeId: number, tasks: Array<{ time: string; taskname: string }>) => {
    try {
      ...
      const { data, error } = await supabase.from("challenge_task").insert(inserts)
          ...
          <ScrollView>
            {challengeList.map((item, index) => (
              <TimeTableBody key={item.id}>
                <TimeInput value={item.time} onChangeText={(value: string) => onChangeItemInput(item.id, "time", value)} />
                <ChallengeInput value={item.taskname} onChangeText={(value: string) => onChangeItemInput(item.id, "taskname", value)} />
